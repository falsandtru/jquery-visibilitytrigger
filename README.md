...
+ visibilitytriggerの実行頻度の制御（`interval``delay``suspend`）。
...
コンテキストを追加します。`$( Element )``$().add( Element )`のショートカットです。コンテキストがある状態でvtを実行した場合、jQueryの`.end()`メソッドにより実行前の選択状態に戻ることができます。
...
コールバック関数（正確にはコールバック関数を実行するかを決めるトリガー要素のチェック）がスクロールイベント発生後実行されるまでの待機時間をミリ秒で設定します。待機時間が経過する前に新たなスクロールが行われた場合は前回までのスクロールによる待機中のコールバック関数の実行はキャンセルされます。jQueryの`trigger()`メソッドによりvisibilitytriggerイベントを直接発生させた場合は`delay`の設定にかかわらずただちに実行されます。`setTimeout`を使用しているため`50`以上の数値を使用することを推奨します。初期値は`300`です。
