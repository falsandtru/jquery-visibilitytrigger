...
s+m>=f-x&&(a.window?!0:v+r>=f-x),b=a.window?!1:n&&v>f+w+x,p=m<=f+w+q&&(a.window?!0:v<=f+w+q),v=a.window?!1:p&&v+r<f-q,b=a.turn&&a.multi&&(1===a.direction?m-a.distance+s>f-l:m+a.distance<f+w+l)?!1:a.skip?n&&p:1===a.direction?n&&!b:p&&!v}}b&&(jQuery.data(k[0],a.nss.data+"-fired",!0),a.count+=1,a.callback.apply(k[0],[c,a.parameter,{index:a.index,length:e.length,direction:a.direction}]));if(!e.length||a.terminate&&!a.multi&&0!==a.step&&a.count>=e.length){n=0;jQuery(d).unbind(a.nss.displaytrigger);jQuery(h).unbind(a.nss.scroll).unbind(a.nss.resize);
