 ...
 */
(function(d){var n=window,E=document,z=void 0,u=["settings"];d.fn.displaytrigger=d.displaytrigger=function(C){function F(e){for(var a=0,c;c=e.context[a];a++)e.id=u.length,e.height=[0,0],u.push(d.extend(!0,{},e)),d(c).unbind(e.nss.displaytrigger).bind(e.nss.displaytrigger,e.id,function(a,b,e){var f=u[a.data],g,c=this,k=arguments.callee;if(f){if(f.delay&&b){for(;g=f.queue.shift();)clearTimeout(g);g=setTimeout(function(){if(f){for(;g=f.queue.shift();)clearTimeout(g);D(a,f,c,b||n)}},f.delay);f.queue.push(g)}else D(a,
f,c,b||n);f.suspend&&!e&&(d(this).unbind(f.nss.displaytrigger),setTimeout(function(){f&&d(c).bind(f.nss.displaytrigger,f.id,k).trigger(f.nss.displaytrigger,[b,!0])},f.suspend))}}),d.data(c,e.nss.data,!0),d(c).unbind(e.nss.scroll).bind(e.nss.scroll,e.id,function(a){(a=u[a.data])&&d(this).trigger(a.nss.displaytrigger,[this])}).unbind(e.nss.resize).bind(e.nss.resize,e.id,function(a){(a=u[a.data])&&d(this).trigger(a.nss.displaytrigger,[this])}),0===a&&d(n).filter(function(){return!e.window&&e.expand}).unbind(e.nss.scroll).bind(e.nss.scroll,
...
r=d(n).scrollTop(),h=a.height[Number(n===x)],t=r===h?a.direction:r<h?-1:1,h=Math.abs(r-h);b&&b.test("++",1,0,"setting");a.direction!==t&&(a.turn=!0,a.end=!1,a.direction=t,a.index=0>a.index?0:l.length<=a.index?l.length-1:a.index,q=l.eq(a.index));a.distance=0===h?a.distance:h;a.height[Number(n===x)]=r;a.end=q[0]?!1:a.end;-1===a.direction&&a.length<l.length&&(a.turn=!1,a.end=!1);a.length=l.length;b&&b.test("++",1,a,"switch");switch(!0){case !l.length:b&&b.test("*",1,0,"case !targets.length");break;case 0===
...
break;default:b&&b.test("*",1,0,"default");if(a.end||q.is(":hidden")||!a.multi&&a.skip&&d.data(q[0],a.nss.data+"-fired"))break;b&&b.test("++",1,0,"initialize");var p=d(x),r=p.scrollTop(),t=p.height(),p=d(k),w=a.window?0:p.offset().top,v=a.window?t:p.outerHeight(!0),h=q.offset().top,y=q.outerHeight(!0),p=Math.max(0,a.direction),A=-1<=a.ahead[0]&&1>=a.ahead[0]?parseInt(t*a.ahead[0]):parseInt(a.ahead[0]),B=-1<=a.ahead[1]&&1>=a.ahead[1]?parseInt(t*a.ahead[1]):parseInt(a.ahead[1]),p=p?B:A,m,s;b&&b.test("++",
...
g=a.turn&&a.multi&&(1===a.direction?r-a.distance+t>h-p:r+a.distance<h+y+p)?!1:a.skip?m&&s:1===a.direction?m&&!g:s&&!w&&a.multi}}b&&b.test("/",1,g,"fire");g&&(!a.multi&&++a.count&&a.skip&&d.data(q[0],a.nss.data+"-fired",!0),a.callback.apply(q[0],[c,a.parameter,{index:a.index,length:l.length,direction:a.direction}]));b&&b.test("++",1,0,"terminate");if(!l.length||a.terminate&&!a.multi&&0!==a.step&&a.count>=l.length){m=0;b&&b.test("*",1,0,"unbind node event");d(k).unbind(a.nss.displaytrigger).unbind(a.nss.scroll).unbind(a.nss.resize);
b&&b.test("++",1,0,"remove data");d.removeData(f,a.nss.data);!a.multi&&a.skip&&l.removeData(a.nss.data+"-fired");for(f=0;l=a.context[f];f++)m+=d.data(l,a.nss.data)?1:0;b&&b.test("++",1,0,"unbind root event");!m&&!a.window&&a.expand&&d(x).unbind(a.nss.scroll).unbind(a.nss.resize);b&&b.end();return u[a.id]=z}b&&b.test("++",1,0,"exit");if(!a.end&&!g&&isFinite(p)&&(1===a.direction?!m:!s))return b&&b.end(),a.turn=!1;b&&b.test("++",1,0,"increment");a.index+=0!==a.step||g?0===a.step&&-1===a.direction?a.direction:
