      ...
      (
        settings , {
          nss : {
            ...
            scrolltrigger : [ 'scrolltrigger' , settings.gns + ( settings.ns ? ':' + settings.ns : '' ) ].join( '.' ) ,
    ...
    .bind( settings.nss.scrolltrigger , settings , function( event ) {
      ...
      if ( !target.length ) {
        if ( settings.terminate ) {
          ...
          for ( var i = 0 ; i < jQuery( settings.scope ).length ; i++ ) {
            remainder += jQuery.data( jQuery( settings.scope ).eq( i ).get( 0 ) , settings.nss.data ) ? 1 : 0 ;
          } ;
      ...
      settings.callback.apply( target.get( 0 ) , [ event , settings.parameter ] ) ;
